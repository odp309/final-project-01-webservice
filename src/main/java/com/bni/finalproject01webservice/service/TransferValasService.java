...
public class TransferValasService implements TransferValasInterface {
    ...
    public DetailTransferValasResponseDTO detailTransferValas(DetailTransferValasRequestDTO request) {
        ...
        if (request.getAmountToTransfer().compareTo(wallet.getCurrency().getMinimumTransfer()) < 0) {
            throw new TransactionException("Amount is less than the minimum transfer!");
        }

        if (request.getAmountToTransfer().compareTo(wallet.getBalance()) > 0) {
        ...
        response.setFirstName(wallet.getUser().getFirstName());
        response.setLastName(wallet.getUser().getLastName());
